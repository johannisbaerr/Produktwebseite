...
function AdminView({ onExit }: { onExit: () => void }) {
  ...
  const moveProduct = async (index: number, direction: -1 | 1) => {
    ...
    if (!neighbor) return
    setReorderError('')
    setMovingProductId(products[index].id)
    try {
      await request('/api/admin/products/reorder', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ productId: products[index].id, neighborId: neighbor.id }) })
      await load()
    } catch (error) {
      setReorderError(error instanceof Error ? error.message : 'Reihenfolge konnte nicht gespeichert werden.')
    } finally {
